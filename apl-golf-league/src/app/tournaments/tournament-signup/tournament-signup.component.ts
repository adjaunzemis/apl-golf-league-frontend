...
export class TournamentSignupComponent implements OnInit, OnDestroy {
  ...
  constructor(private appConfigService: AppConfigService, private tournamentsService: TournamentsService, private golfersService: GolfersService, private formBuilder: FormBuilder, private dialog: MatDialog) { }
  ...
  getDaysUntilSignup(): number {
    return Math.floor((Date.UTC(this.selectedTournament.signup_start_date.getFullYear(), this.selectedTournament.signup_start_date.getMonth(), this.selectedTournament.signup_start_date.getDate()) - Date.UTC(this.currentDate.getFullYear(), this.currentDate.getMonth(), this.currentDate.getDate())) / (1000 * 60 * 60 * 24));
  }
    ...
    dialogRef.afterClosed().subscribe(golferData => {
      if (golferData !== null && golferData !== undefined) {
        ...
        this.golfersService.createGolfer(golferData.name, golferData.affiliation, golferData.email !== '' ? golferData.email : null, golferData.phone !== '' ? golferData.phone : null).subscribe(result => {
          console.log(`[TournamentSignupComponent] Successfully added golfer: ${result.name}`);
