...
export class TeamsService {
  ...
  createTeam(teamData: TeamCreate): Observable<{ id: number, name: string }> {
    ...
    if (teamData.flight_id) {
      return this.http.post<{ id: number, name: string }>(environment.apiUrl + `teams/`, { name: teamData.name, flight_id: teamData.flight_id, golfer_data: teamGolfersSignupData });
    } else {
      return this.http.post<{ id: number, name: string }>(environment.apiUrl + `teams/`, { name: teamData.name, tournament_id: teamData.tournament_id, golfer_data: teamGolfersSignupData });
    }
  }
  ...
  private extractTeamGolferSignupData(teamGolfers: TeamGolferCreate[]): { golfer_id: number, golfer_name: string, division_id: number, role: string }[] {
