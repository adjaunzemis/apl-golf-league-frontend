...
export class FlightSignupComponent implements OnInit, OnDestroy {
  ...
  private getDaysUntilSignupStart(info: FlightInfo): number {
    const timeDiff = info.signup_start_date.getTime() - this.currentDate.getTime();
    return Math.ceil(timeDiff / (1000 * 3600 * 24));
  }

  private getDaysUntilSignupStop(info: FlightInfo): number {
    const timeDiff = info.signup_stop_date.getTime() - this.currentDate.getTime();
    return Math.ceil(timeDiff / (1000 * 3600 * 24));
  }

  getSignupStatusText(flightInfo: FlightInfo) {
  ...
  getSignupStatusColor(flightInfo: FlightInfo) {
    const daysToStart = this.getDaysUntilSignupStart(flightInfo);
    const daysToStop = this.getDaysUntilSignupStop(flightInfo);
    if (daysToStart > 0) {
      return 'info';
